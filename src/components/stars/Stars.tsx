...
import type { Props } from "./Stars.props";

const Stars = ({ rating, className, ...props }: Props) => {
  const currentRating = 5 - rating;
  const ActiveStars = Array.from({ length: rating }).map((_, ind) => (
    <svg
      key={ind}
      width="16"
      height="16"
  ...
  const unactiveStars = Array.from({ length: currentRating }).map(
    (_, index) => (
      <svg
        key={index}
        width="16"
